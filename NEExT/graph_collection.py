...
class Graph_Collection:
    ...
    def load_graphs_from_csv(self, edge_file, node_graph_mapping_file, node_features_file=None):
        ...
        if node_features_file:
            ...
            node_features_map = {}
            for idx in tqdm(range(len(node_features)), desc="Loading node features:",
    ...
    def build_node_sample_collection(self, sample_rate):
        ...
        self.graph_id_node_array = []
        for g_obj in tqdm(self.graph_collection, desc="Building node samples:",
                          disable=self.global_config.quiet_mode):
            g_obj.node_samples = random.sample(
                list(g_obj.graph.nodes), int(sample_rate * len(g_obj.graph.nodes))
            )
