...
class Graph(BaseModel):
    ...
    def initialize_graph(self):
        ...
        if self.graph_type == "networkx":
            ...
            # Warn about sparse node IDs that waste memory
            if max_node + 1 > len(node_set) * 2:
                logger.warning(
                    f"Graph {self.graph_id}: iGraph creating {max_node + 1} vertices for {len(node_set)} nodes. "
                    "Consider reindexing nodes to avoid wasted memory."
                )
    ...
    def _reindex_nodes(self):
        ...
        # Map node attributes
        new_node_attrs = {node_mapping[node]: attrs for node, attrs in self.node_attributes.items()}

        # Map edge attributes
        new_edge_attrs = {(node_mapping[src], node_mapping[dst]): attrs for (src, dst), attrs in self.edge_attributes.items()}
    ...
    def _filter_largest_component(
        ...
        List[int], List[Tuple[int, int]], Dict[int, Dict[str, Union[float, int, str]]], Dict[Tuple[int, int], Dict[str, Union[float, int, str]]]
    ]:
        if self.graph_type == "networkx":
            ...
            node_attrs = {n: {k: v for k, v in subgraph.nodes[n].items()} for n in nodes if subgraph.nodes[n]}

            edge_attrs = {e: {k: v for k, v in subgraph.edges[e].items()} for e in edges if subgraph.edges[e]}
